...
import { useGetActiveUsers } from '../services/apis/user';
...
export default function AllActiveUser() {
    const [limit] = useState(100);
    const [page, setPage] = useState(1)
    const { data: users, isLoading, isError, error, isFetching, isPreviousData } = useGetActiveUsers({ page, limit });
    ...
    return (
        <DashboardLayout isOpen={value} toggleValue={toggleValue}>
            ...
            <nav className="w-full bg-[#0084C7] px-4 sm:px-12 py-4 h-[360px]">
                <Navbar isOpen={value} toggleValue={toggleValue} title='active users' />
            </nav>
